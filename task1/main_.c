...
#define MAX_USERS  283228//  610
#define LINE 1024
#define BOOKS_INPUT 58098 //58098 9742
#define BUFFER 200

char *iFiLeName = "ratmed.txt";
char *iMovName = "movmed.txt";
...
float queryDotProductBetween(struct SparseMatrixLinkedList *root, int src, int to) {
  //if (dpDot[src][to] || dpDot[to][src])
  //  return dpDot[src][to];

  struct Pair res = dotProductBetween(root->indexForRoots[src], root->indexForRoots[to]);
...
float queryPearsonCoef(struct SparseMatrixLinkedList *root, int src, int to) {
  //float f = queryDotProductBetween(root, src, to);
  //float s = (root->indexForRoots[src]->sum * root->indexForRoots[to]->sum)/dpTamDot[src][to];
  //float t = sqrtf(root->indexForRoots[src]->module - (pow(root->indexForRoots[src]->sum, 2)/dpTamDot[src][to]));
  //float q = sqrtf(root->indexForRoots[to]->module - (pow(root->indexForRoots[to]->sum, 2)/dpTamDot[src][to]));
  return 1 /*(f - 1/t * q)*/;
}
